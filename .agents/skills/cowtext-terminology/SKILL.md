...
description: Canonical Cowtext vocabulary — module map, the invoke command list, the four Tauri events, and the terms every agent must use consistently. Load before writing code, docs, or reports so names stay byte-exact. Full definitions live in docs/TERMINOLOGY_REFERENCE.md.
---
...
**Never write a count into prose.** Command counts, test counts and the schema version
live in one generated place — the truth block in `CLAUDE.md` between `<!-- truth:begin -->`
and `<!-- truth:end -->`, written by `npm run truth:write`. `npm run truth` fails on a
stale number in this file too; it is mirrored byte-for-byte to `.agents/skills/`, so edit
this copy and run the script, never the mirror.

**Provider support, verbatim on every surface:** "Cowtext compiles context for multiple
AI coding agents. Assemble, Run and live hooks currently use Claude Code." Never paraphrase
it and never widen it — compile has five targets, but Assemble, Run, handoff, agent files,
skills and hooks are Claude Code only. Capability detail: `docs/design/PROVIDER_SUPPORT_MATRIX.md`.

## Module map

| Module | Owns |
|---|---|
| `src-tauri/src/lib.rs` | Builder chain, plugin registration, the `generate_handler!` list (count: truth block) |
...
- **Built-in skill** — a skill the app bundles (`src/resources/skills/<id>/SKILL.md`) in one of
  three states: **virtual** (bundled only, nothing on disk), **materialized** (written to the
  project's `.claude/skills/<id>/SKILL.md` by `skills_materialize`, byte-equal to the bundle),
  **modified** (on disk and different — it becomes a project skill with a `modified from
  built-in` badge and a Reset action). Compile writes included **virtual** built-ins and never
  a **modified** one.
- **Truth block** — the generated lines in `CLAUDE.md` between `<!-- truth:begin -->` and
  `<!-- truth:end -->`; the only place counts live. Written by `npm run truth:write`.
- **Ports** — 1420 Vite dev (strictPort, pinned in two files); 4923 hooks server, exposed to
  the UI by `hooks_addr` so no component hard-codes the address.

...
- `docs/` layout: `TERMINOLOGY.md` is the canon (module map, the invoke commands (54), the four Tauri events, key terms — keep ≤ 120 lines); full definitions live in `TERMINOLOGY_REFERENCE.md`. `tasks/` holds exactly TASKS / BACKLOG / BUGS / ROADMAP (milestones and the sprint log are sections inside ROADMAP.md; the feature inventory is a section inside BACKLOG.md). `design/` = design specs, `testing/` = manual test scripts, `fleet/` = ROSTER.md + ACTIVITY_LOG.md (three most recent sessions only).
...
**V2 REPLAN IN EFFECT (2026-08-19)** — positioning: *Cowtext is a context compiler for AI coding agents*; four-layer plan (L1 graph / L2 orchestrator / L3 workflows / L4 observability+barn) in ROADMAP.md, strategy per `Cowtext_Strategy_2026.pdf`. All seven phases (0–6) accepted; WO01 closed; WO02 committed `103ac80`; **WO03 LANDED** (L1 moat hardening: 7 lanes A–G, 54/54 invoke contract, all gates green; graph v3 with 13 roles/7 edge kinds/tags+owner+meta+color, compile targets 3→5, `cowtext-cli`, importer MVP, linter v1; 14 defects found/fixed, 2 CRITICAL; 5 audit OBSERVATIONS logged to backlog; manual written 71 steps A–H). v0.1.0 cut gate = Marty's acceptance walk. **Next: WO04 — L1 completion** (hierarchy, symlink manager, SKILL.md target, full import, resolved-context preview, context loadouts, branch-aware, preset packs, GitHub Action). Open: WO01 walk (A–F+N), WO02 walk (61 steps), WO03 walk (71 steps).

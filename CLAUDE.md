...
- `docs/` layout: `TERMINOLOGY.md` is the canon (module map, the invoke commands (63), the four Tauri events, key terms — keep ≤ 120 lines); full definitions live in `TERMINOLOGY_REFERENCE.md`. `tasks/` holds exactly TASKS / BACKLOG / BUGS / ROADMAP (milestones and the sprint log are sections inside ROADMAP.md; the feature inventory is a section inside BACKLOG.md). `design/` = design specs, `testing/` = manual test scripts, `fleet/` = ROSTER.md + ACTIVITY_LOG.md (three most recent sessions only).
...
**V2 REPLAN IN EFFECT (2026-08-19)** — positioning: *Cowtext is a context compiler for AI coding agents*; four-layer plan (L1 graph / L2 orchestrator / L3 workflows / L4 observability+barn) in ROADMAP.md, strategy per `Cowtext_Strategy_2026.pdf`. All seven phases (0–6) accepted; WO01 closed; WO02 committed `103ac80`; WO03 LANDED (L1 moat hardening: graph v3, importer, linter, cowtext-cli); WO06 LANDED (L2 orchestrator: task DAG + tasklinks + subgraph injection + budgets + mission control); WO09 LANDED (canvas connectors round 2); WO10 LANDED (canvas v4 + project suite); **WO11 LANDED 2026-08-20** — acceptance-walk defects A–G (Home button, Git init + .gitignore, project Inspector panel, agent avatars, fleet detail), three doctrines (ONE_WRITER per file, TOCTOU serialization via `AGENT_FS` mutex, standing path-safety rule), all 16 defects found in audit fixed. Invoke **66→73** (73/73 reachable), **604 Rust tests** (586 lib + 18 CLI), all gates green. Three crashes remain UNPROVEN and open (B2 agent selection, C1 adopt-to-graph, F1 assemble-after-close); all confirmed code defects on those paths are fixed; ErrorBoundary installed. True sequence: WO03 ✅ → WO06 ✅ → WO09 ✅ → WO10 ✅ → **WO11 ✅** → WO04 (contract ready, not built) → WO05 → WO07. **Next**: Marty walks WO11 + WO10 + WO09 + WO03 + WO02 + WO01 manuals (all open); decide WO04 start. Still open from 08/19: real global edge router (partial in WO10). v0.1.0 cut gate = acceptance walks + real sprites (Marty-side).

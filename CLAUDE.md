...
- `docs/` layout: `TERMINOLOGY.md` is the canon (module map, the invoke commands (76), the four Tauri events, key terms — keep ≤ 120 lines); full definitions live in `TERMINOLOGY_REFERENCE.md`. `tasks/` holds exactly TASKS / BACKLOG / BUGS / ROADMAP (milestones and the sprint log are sections inside ROADMAP.md; the feature inventory is a section inside BACKLOG.md). `design/` = design specs, `testing/` = manual test scripts, `fleet/` = ROSTER.md + ACTIVITY_LOG.md (three most recent sessions only).
...
**TITLE SCREEN REDESIGN (2026-08-21)** — `src/project/TitleScreen.tsx` replaces App.tsx's inline `EmptyState`/`RecentProjects` (App.tsx −208 lines): brand lockup (16×14 pixel cow, Silkscreen wordmark), three doors, recents list, and an **AI toolchain** panel scanned on demand by the new `detect_ai_tools` invoke (`src-tauri/src/toolchain.rs` — PATH probe + `--version` per compile target, concurrent, 4s timeout each, `gh copilot` needs its version call to succeed). `AiToolchainModal` ticks write `defaultCompileTargets` in app settings, which `graph.ts::loadGraph` seeds a brand-new project from (also fixes targets leaking across a project switch). Two compositions: two-column with recents, centred first-run without. Invoke **75→76**; **785 Rust tests** (751 lib + 18 CLI + 16 MCP); 163 frontend tests; `tsc`/`clippy` clean; `eslint` 0 errors. Verified in the running app (both compositions). Design canvas: https://claude.ai/code/artifact/bc66667a-7bcb-4dbe-9265-fe6edde416d7

...
- `docs/` layout: `TERMINOLOGY.md` is the canon (module map, 23 invoke commands, events, key terms — keep ≤ 120 lines); full definitions live in `TERMINOLOGY_REFERENCE.md`. `tasks/` holds exactly TASKS / BACKLOG / BUGS / ROADMAP (milestones and the sprint log are sections inside ROADMAP.md; the feature inventory is a section inside BACKLOG.md). `design/` = design specs, `testing/` = manual test scripts, `fleet/` = ROSTER.md + ACTIVITY_LOG.md (three most recent sessions only).
...
ALL SEVEN PHASES (0–6) CODE-COMPLETE AND ACCEPTED — Phases 0–2 accepted incrementally (Phase 2: 2026-08-16), Phase 3–6 acceptance walks passed 2026-08-18; milestones M0–M6 all reached. Current version **v0.0.0007** = the 2026-08-17 ship-prep landing (`cb770d4`): production CSP + relaxed devCsp (`tauri.conf.json` pre-bumped to 0.1.0), code-split 1,334 → 207 kB, ESLint 10 + `npm run lint`, `pixi.js/unsafe-eval` CSP fix. Gates green: build, lint (0 errors/1 warning), clippy `-D warnings`, cargo test 88/88, invoke contract 23 commands. **Next version: v0.1.0** (first public cut) — remaining: real sprites (Aseprite originals per `docs/design/ART_DIRECTION.md`, Marty-side, BACKLOG P1), CSP runtime check under a production build (TASKS), then the tagged cut. 2026-08-18 docs+fleet session: docs/ reorganized (13 files ~207 KB archived via git mv into `docs/_archive/`; FEATURES→BACKLOG Feature inventory, MILESTONES+SPRINTS→ROADMAP; TERMINOLOGY split into canon + REFERENCE, invoke count corrected 14→23); 6+1 agent fleet in `.claude/agents/` + 5 knowledge skills + `/ultracode` dispatcher; tasks docs actualized to all-phases-accepted (ROADMAP: merged phases+milestones table; TASKS: walks closed, single history table; BUGS: fixed sections rolled off; BACKLOG: open/completed split, sprites raised to P1). Docs session uncommitted.

...
Phase: 2 accepted 2026-08-16. Phases 3+4 built by the fleet same day (v0.0.0004): `src-tauri/src/assemble.rs` (queue, max-2 concurrency, Runner seam, prompt piped to `claude -p` over stdin), `hooks.rs` (trust-boundary preview/write, never-clobber merge), `hooks_server.rs` (axum 127.0.0.1:4923 → `barn://event`), 7 new commands; frontend `src/store/events.ts`, `src/assemble/`, EventLog, HooksModal, Inspector assemble section, live node pulse. Adversarial audit found 6 real defects (1 critical: Windows claude spawn) — all fixed and verified. Gates green: `npm run build`, `cargo clippy --all-targets -- -D warnings`, `cargo test` 60/60, invoke-name contract (14 commands). NOT yet manually accepted — walk `docs/testing/PHASE3_TEST_MANUAL.md` and `docs/testing/PHASE4_TEST_MANUAL.md` before closing. Barn prototype started early per Marty: `src/scene/` (Pixi 8 iso scene, demo mode, Canvas ⇄ Barn view toggle) — test via `docs/testing/BARN_PROTO_TEST_MANUAL.md`.

...
| **Session / Run runtime** | ✓ headless `claude -p` resume loop → `agent://event` | ✗ | ✗ | ✗ | ✗ | `sessions.rs` (`agent://event`, `docs/TERMINOLOGY.md:69`); `App.tsx:505` |
...
| **MCP** | `cowtext-mcp` stdio server (14 tools) — usable by **any** MCP client; registered for Claude Code in `.mcp.json` and for Codex in `.codex/config.toml` — both in this repo only, both pointing at an absent release exe (§4) | same binary | any MCP client | any MCP client | any MCP client | `src-tauri/src/bin/cowtext_mcp.rs`, `.mcp.json:5`, `.codex/config.toml:2` |
...
| `.codex/agents/*.toml` (5 files) | `name` / `description` / `developer_instructions` TOML, text copied from `.claude/agents/*.md` with blind substitutions (`AGENTS.md hard rules`, `AGENTS.md §Status`) | **(d) unverified Codex agent format, dev-only.** Nothing in the product reads or writes them. | Not deleted until Marty confirms (e). If kept, they are regenerated by a future script — out of WO15 scope. |
...
| Run button tooltip | `src/App.tsx:505` — "Run — launches a headless Claude Code session (claude -p)" | U4a | manual |
...
| EventLog empty copy | `src/inspector/EventLog.tsx:241` (already names Claude Code; keep) | U1 | manual |

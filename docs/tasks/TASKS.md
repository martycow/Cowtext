...
| WO01 Block B+C+F review walk | wo01, block-b, block-c, block-f, qa, acceptance | Review `docs/testing/WO01_BLOCK_A_TEST_MANUAL.md` appended sections B/C/F (15 min total): Block B token budget (chips, bars, consts), Block C review queue (selfWrite suppress, snapshots, side-by-side diff; 1 MAJOR skipCurrent defect FIXED pre-landing), Block F Agents MVP (headless spawn resume-loop, roster bar, agent panel, kill/restart; process-tree audit: 2 found, 0 remaining). Invoke contract 43→50 verified. All gates green. Uncommitted code awaits approval. | P0 | 2026-08-18 | 🔲 Open |

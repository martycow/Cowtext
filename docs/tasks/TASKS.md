...
All seven phases (0–6) are accepted as of 2026-08-18; WO01 is now fully closed (Blocks A–F landed + N1–N5 uncommitted). Only v0.1.0 release work remains open.

## Open — toward v0.1.0

| Name | Tags | Description | Priority | Date Created | Status |
|---|---|---|---|---|---|
| WO01 full acceptance walk (manual sections A–F + N) | wo01, qa, acceptance | Marty acceptance walk: review all sections of `docs/testing/WO01_BLOCK_A_TEST_MANUAL.md` (Blocks A–F: 29 + 15 min) and N-suite manual steps covering task-format skill, @mention-chip integration, manager mode, status bar. All gates green (50 invoke commands, cargo tests passing, tsc/lint clean). Two known-issues documented from Block A audit: MAJOR Windows notify rename ghost-entries, MINOR TOCTOU race in flush() generation guard. Approve for v0.1.0 cut or mark as v0.1.1 follow-up. | P0 | 2026-08-18 | 🔲 Open |
...
|---|---|---|---|---|
| 2026-08-18 WO01 fully closed (v0.1.0) | Blocks A–F + N1–N5 | wo01, ultracode, product | 7-block ultracode work order: Blocks A (canvas lenses+watcher, reviewed+accepted), B (token budget, `f499ccf`), C (review queue, `306f56a`), D (`72dc1a9`), E (`f1752b0`), F (Agents MVP, `269bdad`); N1–N5 task-format skill batch (uncommitted). Invoke 43→50 verified. 1 N-batch defect found and fixed in tester walk. All gates green. | ✅ Done — awaiting v0.1.0 acceptance walk |

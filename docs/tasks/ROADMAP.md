...
|---|---|---|---|
| 1 | **Golden-path walk** | Marty walks `docs/testing/GOLDEN_PATH_MANUAL.md` end to end in one sitting — 38 risk-based scenarios, sections A–J, ~55 min, with the *Snap before / Snap after* folder-watch rule on every writing step. Prerequisite: `git` on PATH with a global identity. | **OPEN** — manual authored 2026-08-22, never walked |
| 2 | **Acceptance walks** | The per-work-order manuals in `docs/testing/` that Marty has not yet walked: WO13, WO12, WO11, WO10, WO09, WO03, WO02, WO01. Findings go back to their own work order, not to a new one. | **OPEN** — eight manuals outstanding |
